...
postpuesto(customerData)
 {
   this.http.post(Constantes.capiURL+"Area",customerData/*,  { headers: { Authorization:localStorage.getItem('token') } }*/).subscribe((res: Response) =>
     {
 ...
 getpuesto(index)
 { 
   this.http.get(Constantes.capiURL+"Area"+'/'+index).subscribe(data =>
     {console.log(data);
 ...
 {  
   ...
   this.http.post(Constantes.capiURL+"Area"+'/'+idd, customerData).subscribe(data =>
     {console.log(data);
